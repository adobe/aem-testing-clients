...
/**
 * Create a new page. This rule can be sub-classed to specify the parent page and the template of
 * the newly created page. Subclasses can also specify which client to use or which server to target
...
public class Page extends ExternalResource {
  ...
  // singleton value; true if the target test instances have been prepared once since the class was
  // loaded
  ...
  private ThreadLocal<String> parentPath =
  ...
  private ThreadLocal<String> templatePath =
      new ThreadLocal<String>() {
        @Override
        protected String initialValue() {
  ...
  protected void before() throws ClientException {
    ...
    SlingHttpResponse response =
        getClient()
            .createPage(
                getName(), getTitle(), getParentPath(), getTemplatePath(), HttpStatus.SC_OK);
  ...
  /**
   * The initial parent path to be used when creating the page This implementation returns {@value
   * SITE_ROOT_PATH} You can override this with dynamic names for each threads
   *
  ...
  /**
   * The initial page name to be used when creating the page This implementation returns
   * "testpage_[randomUUID]" You can override this with dynamic names for each thread
   *
  ...
  /**
   * The initial template path to be used when creating the page This implementation returns {@value
   * TEMPLATE_PATH} You can override this with dynamic names for each threads
   *
  ...
  /**
   * The client to use to create and delete this page. The default implementation creates a {@link
   * CQClient}. The default implementation also uses the default admin user.
   *
  ...
  /**
   * Method to be called before creating the page. You can override this to perform custom
   * operations before creating the page. This implementation creates, if needed, the test template
   * and site.
   *
  ...
  /**
   * The node name of this page. The default implementation returns the value of {@link
   * #initialName()}.
   *
  ...
  /**
   * The path of the parent of this page. The default implementation returns the value {@link
   * #initialParentPath()}
   *
  ...
  /**
   * The path of the template of this page. The default implementation returns the value {@link
   * #initialTemplatePath()}
   *
