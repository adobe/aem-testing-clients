...
public class User extends AbstractAuthorizable {
   ...
   *
   * <p>By default it will wait {@code 6000}ms but it can be fine tuned with, for example {@code
   ...
   */
  public <T extends User> SlingHttpResponse addImpersonators(
      T[] authorizables, int... expectedStatus) throws ClientException, InterruptedException {
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public <T extends User> SlingHttpResponse removeImpersonators(
      T[] authorizables, int... expectedStatus) throws ClientException {
    if (authorizables == null) {
      throw new IllegalArgumentException("List of authorizables may not be null!");
    }
    // add user to group - base = group
    ...
    formEntry.addParameter(Constants.PARAMETER_CHARSET, Constants.CHARSET_UTF8);
    for (T authorizable : authorizables) {
      formEntry.addParameter(
          Authorizable.PARAM_REMOVE_IMPERSONATORS, encodeURI(authorizable.getId()));
    }
  ...
  public static <T extends SecurityClient> User createUser(
      T client,
      String userId,
      String password,
   ...
   * @throws ClientException If something fails during request/response cycle
   * @throws InterruptedException to mark this method as "waiting"
   * @deprecated waitForIndex parameter is not needed. Use {@link #createUser(SecurityClient,
   *     String, String, String, Map, int...)}
   */
  @Deprecated
  public static <T extends SecurityClient> User createUser(
      T client,
