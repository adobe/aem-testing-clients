...
public class WorkflowClient extends CQClient {
  ...
  public enum Status {
     ...
     *
     * @param status the string associated with this enum value
     */
  ...
  /**
   * Creates a new workflow model in the repository by sending a model definition.<br>
   * <br>
   * Handled by:<br>
   * <br>
   ...
   *
   * @param modelSourcePath path to model
   * @param modelType if null or MODEL_DESCR_TYPE_JSON, the model description should be in JSON
   *     format.
   * @param expectedStatus list of allowed HTTP Status to be returned. if not set, http status 201
   *     (CREATED) is assumed.
   ...
   * @throws IOException if access to to the JSON resource fails
   */
  public String deployModel(String modelSourcePath, String modelType, int... expectedStatus)
      throws ClientException, IOException {
    // check for default value
    if (modelType == null) modelType = MODEL_DESCR_TYPE_JSON;
  ...
  /**
   * same as {@link #deployModel(String, String, int...)}, but with a list of strings to replace in
   * the model
   *
  ...
  public String deployModel(
      String modelSourcePath, String modelType, List<NameValuePair> replaces, int... expectedStatus)
      throws ClientException, IOException {
    // check for default value
    if (modelType == null) modelType = MODEL_DESCR_TYPE_JSON;
    // get the JSON definition
    ...
    for (NameValuePair replacePair :
        (replaces != null) ? replaces : new ArrayList<NameValuePair>(0)) {
   ...
   *
   * <p>Handled by: granite\bundles\workflow\console\src\main\java\com\adobe\granite
   * \workflow\console\servlet\InboxServlet.java
   *
   * @param workItemId the work item to advance
   * @param routeId the route it should take
   ...
   * @throws ClientException if HTTP request fails for any reason.
   */
   ...
   *
   * @param workflowInstancePath the source of the workflow instance
   * @param chooseRoute based on the model at the expected step, the route that should be used (0,
   *     1, 2, etc..)
   * @return The request executor as list if access to response is needed for each of the workitems
   *     (in case more than one was found).
   * @throws ClientException if HTTP request fails for any reason.
   */
   ...
   *
   * @param workItemId the work item to advance
   * @param backrouteId the route it should take
   * @param expectedStatus The HTTP status that should be returned, otherwise 200 is assumed.
   * @return The request executor if access to response is needed.
   * @throws ClientException if HTTP request fails for any reason.
   */
  public SlingHttpResponse stepBackWorkItem(
      String workItemId, String backrouteId, int... expectedStatus) throws ClientException {
    SlingHttpResponse exec =
        doPost(
            MANAGE_WF_INBOX_PATH,
            FormEntityBuilder.create()
   ...
   * \workflow\console\servlet\InboxServlet.java
   *
   * @param workItemId the work item to delegate
   * @param delegatee the user/group this work item gets delegated to
   * @param expectedStatus The HTTP status that should be returned, otherwise 200 is assumed.
   * @return The request executor if access to response is needed.
   * @throws ClientException if HTTP request fails for any reason.
   */
  public SlingHttpResponse delegateWorkItem(
      String workItemId, String delegatee, int... expectedStatus) throws ClientException {
    SlingHttpResponse exec =
        doPost(
            MANAGE_WF_INBOX_PATH,
            FormEntityBuilder.create()
                .addParameter("item", workItemId)
                .addParameter("delegatee", delegatee)
                .build());
    // check return status
    HttpUtils.verifyHttpStatus(exec, HttpUtils.getExpectedStatus(SC_OK, expectedStatus));
    // return executor
   ...
   * @param routeId the user/group this work item gets delegated to
   * @param expectedStatus The HTTP status that should be returned, otherwise 200 is assumed.
   * @return The request executor if access to response is needed.
   * @throws ClientException if HTTP request fails for any reason.
   */
  public SlingHttpResponse approveAndAdvanceWorkItem(
      WorkItem workItem, String routeId, int... expectedStatus) throws ClientException {
  ...
  /**
   * Returns the list of work item that are currently in the inbox of this client's user. The
   * returned map is using the work items uri as key.
   *
   * <p>handled by: granite\bundles\workflow\console\src\main\java\com\adobe\granite
   * \workflow\console\servlet\InboxServlet.java
   *
   * @param expectedStatus The HTTP status that should returned, othewise 200 is assumed.
   * @return A map where key ist the uri of the stored work item
   * @throws ClientException if requesting the JSON fails
   */
  ...
  public List<String> getWorkItemBackRouteIds(String uri, int... expectedStatus)
      throws ClientException {
    // make the request
  ...
  /**
   * Deletes a workflow model definition. Note that the model node does not get removed. It only
   * gets a property set in its Metadata sub node named 'deleted' with a value of true.<br>
   * <br>
   * In order to solve firewall/proxy issues a {@code POST} that contains a {@code
   * X-HTTP-Method-Override} header entry with value {@code DELETE} is sent instead of the proper
   * {@code HTTP DELETE}.<br>
   * <br>
   * Handled by:<br>
   * <br>
   ...
   * @param expectedStatus HTTP status to be returned, otherwise 20 (NO CONTENT) is assumed
   * @throws ClientException if POST request fails for any reason
   * @return the request executor containing the response object and content.
   */
  public SlingHttpResponse deleteModel(String modelId, int... expectedStatus)
      throws ClientException {
   ...
   */
  public SlingHttpResponse abortWorkflow(String instanceId, int... expectedStatus)
      throws ClientException {
    ...
    SlingHttpResponse exec =
        doPost(
            instanceId,
            FormEntityBuilder.create()
                .addParameter("state", "ABORTED")
                .addParameter("terminateComment", "")
  ...
  /**
   * updates an existing workflow model in the repository by sending a updated model definition.<br>
   * <br>
   * Handled by:<br>
   * <br>
   * granite\bundles\workflow\console\src\main\java\com\adobe\granite\workflow
   ...
   * @param modelType if null or MODEL_DESCR_TYPE_JSON, the model description should bE in JSON
   *     format.
   * @param modelId the model to be updated
   ...
   * @throws IOException if access to to the JSON resource fails
   */
  public SlingHttpResponse updateModel(
      String modelSourcePath, String modelType, String modelId, int... expectedStatus)
      throws ClientException, IOException {
    // check for default value
    if (modelType == null) modelType = MODEL_DESCR_TYPE_JSON;
    // get the JSON definition
   ...
   *
   * @param expectedStatus The HTTP Status to be returned otherwise 200 is assumed
   * @return the Workflow Engine wrapper
   * @throws ClientException if the request for the JSON data fails.
   */
   ...
   * <p>Handled by:
   *
   * <p>granite\bundles\workflow\console\src\main\java\com\adobe\granite\workflow
   * \console\servlet\EngineServlet.java
   *
   * @param expectedStatus The HTTP Status to be returned otherwise 200 is assumed
   * @return returns the request executor, if more info from the response is required.
   * @throws ClientException if the POST request fails.
   */
  ...
  /**
   * Checks if the workflow engine is in an active state (actually its the workflow OSGi service
   * that gets tested).
   *
   * @return true if the Workflow Engine is active otherwise false.
   * @throws ClientException if requesting Workflow Info fails
   */
   ...
   *
   * @param modelId the workflow model id to be used.
   * @param payload the payload JCR path to be set.
   * @param expectedStatus The HTTP Status to be returned otherwise 201 is assumed
   * @return the id of the new workflowInstance
   * @throws ClientException if the POST request fails
   */
  public String startWorkflow(String modelId, String payload, int... expectedStatus)
      throws ClientException {
    return startWorkflow(modelId, PAYLOAD_TYPE_JCR, payload, null, null, null, expectedStatus);
  }

  /**
   * Starts a new workflow using the given model id, with the payload specified.
   *
   * <p>Handled by:
   *
   * <p>granite\bundles\workflow\console\src\main\java\com\adobe\granite\workflow
   ...
   *
   * @param modelId The workflow model Id to be used.
   * @param payloadType The type of payload referencing, either {@link #PAYLOAD_TYPE_JCR} or {@link
   *     #PAYLOAD_TYPE_URL} . if set to null , {#PAYLOAD_TYPE_JCR} is used as default
   * @param metaData any meta data that needs to be passed when starting the workflow
   * @param title title to be set for this workflow instance
   * @param comment comment to be set when starting the workflow
   * @param expectedStatus The HTTP Status to be returned otherwise 201 is assumed
   * @param payload the JCR path(s) or URL(s) pointing to the server resource that act as payload.
   *     each payload entry will start a new workflow instance
   * @return the id of the new workflowInstance
   * @throws ClientException if the POST request fails
   */
  public String startWorkflow(
    ...
    if (modelId == null || "".equals(modelId)) throw new ClientException("Invalid ModelId!");
    // if no payload is given , fail
    if (payload == null || "".equals(payload)) throw new ClientException(("No payload set!"));
    // set the default for the type if needed
    if (payloadType == null) payloadType = PAYLOAD_TYPE_JCR;
   ...
   *
   * @param instanceURI the workflow instance to be updated
   * @param comment the comment of the workflow instance to be set
   * @param expectedStatus The HTTP Status to be returned otherwise 200 is assumed
   * @return the request executor containing the response object and content.
   * @throws ClientException if the POST request fails.
   */
  public SlingHttpResponse updateWfInstanceComment(
      String instanceURI, String comment, int... expectedStatus) throws ClientException {
   ...
   *
   * @param instanceURI the workflow instance to be updated
   * @param title the title of the workflow instance to be set
   * @param expectedStatus The HTTP Status to be returned otherwise 200 is assumed
   * @return the request executor containing the response object and content.
   * @throws ClientException if the POST request fails.
   */
  public SlingHttpResponse updateWfInstanceTitle(
      String instanceURI, String title, int... expectedStatus) throws ClientException {
    HashMap<String, String> map = new HashMap<>();
   ...
   *
   * @param instanceURI the workflow instance to be updated
   * @param metaData the metadata to be added /updated
   * @param expectedStatus The HTTP Status to be returned otherwise 200 is assumed
   * @return the request executor containing the response object and content.
   * @throws ClientException if the POST request fails.
   */
  public SlingHttpResponse updateWfInstanceMetaData(
      String instanceURI, Map<String, String> metaData, int... expectedStatus)
      throws ClientException {
    // build the form entity
    FormEntityBuilder form = FormEntityBuilder.create();
  ...
  /**
   * Returns a list of workflow instance URI's that are visible for the user. Can be filtered by the
   * instance status.<br>
   * <br>
   * Handled by :<br>
   * <br>
   ...
   *
   * @param wfStatus Only returns workflows that are in one of the possible States ( {@link
   *     Status#RUNNING}, {@link Status#ABORTED}, {@link Status#COMPLETED} or {@link
   *     Status#SUSPENDED}), or, if set to null, returns all workflow instances.
   * @param expectedStatus HTTP Status to check for or, if not set, 200 is assumed
   * @return A String List containing all workflow instance handles visible to this user according
   *     to the filter set in {@code wfStatus}.
   * @throws ClientException if the call to the backend requesting the JSON fails
   */
  public List<URI> getWorkflowInstanceURLs(Status wfStatus, int... expectedStatus)
      throws ClientException {
    // default Status
    String status = "";
  ...
  /**
   * Returns a list of all available workflow models for this user. Handled by:
   * granite\bundles\workflow\content\src\main\content\jcr_root\libs\cq\
   * workflow\components\models\json.jsp
   *
   * @param listType if set to MODEL_LIST_TYPE_ID , a list of model ID's gets returned (model ID =
   *     model Handle) if set to null or MODEL_LIST_TYPE_URI, a list of model URI's gets returned
   *     (URI = context path + model ID)
   * @param expectedStatus HTTP Status to check for or, if not set, 200 is assumed
   * @return A String array containing a list of all workflow model visible to this user
   * @throws ClientException if requesting the JSON fails for any reason
   */
  public ArrayList<String> getWorkflowModels(String listType, int... expectedStatus)
      throws ClientException {

    // set default value for listType if not set
    if (listType == null) listType = MODEL_LIST_TYPE_URI;

    // execute the request
    SlingHttpResponse exec =
        doGet(
            MANAGE_WF_MODELS_PATH + ".json",
    ...
    if (listType.equals(MODEL_LIST_TYPE_ID)) nodeProperty = "value";
  ...
  /**
   * Returns the model definition (metadata, nodes, transitions,title ,description etc)
   *
   * <p>handled by: \granite\bundles\workflow\console\src\main\java\com\adobe\granite
   * \workflow\console\servlet\ModelServlet.java
   *
   * @param modelId the model id to request
   * @param version the version to retrieve. Either 'HEAD' or null to get the latest version or in
   *     the format major.minor e.g. '1.2'.
   * @param expectedStatus the expected HTTP Status, if not set 200 is assumed
   * @return the requested version of the model definition in JSON format
   * @throws ClientException If requesting the JSON fails for any reason
   */
  public String getWorkflowModelAsJSON(String modelId, String version, int... expectedStatus)
      throws ClientException {
  ...
  /**
   * Returns the model definition (metadata, nodes, transitions,title ,description etc) in a wrapper
   * class.
   *
   * <p>handled by: \granite\bundles\workflow\console\src\main\java\com\adobe\granite
   * \workflow\console\servlet\ModelServlet.java
   ...
   * <p>Handled by:
   *
   * <p>granite\bundles\workflow\content\src\main\content\jcr_root\libs\cq\
   * workflow\components\instance\json.jsp
   *
   * @param instanceURI the path to the workflow instance
   * @param expectedStatus the expected HTTP status, otherwise 200 is assumed
   * @return A WorkflowInstance that wraps the JSON response.
   * @throws ClientException if requesting the JSON fails
   */
  public WorkflowInstance getWorkflowInstance(String instanceURI, int... expectedStatus)
      throws ClientException {
    // get the data
  ...
  public List<WorkflowInstance> getWorkflowInstances(
      Status wfStatus, String model, String payload, DateTime startTimeLimit, DateTime endTimeLimit)
      throws ClientException {
    ...
    for (URI url : urls) {
      ...
      if (model != null && !workflowInstance.getModelId().equals(model)) continue;
      if (payload != null && !workflowInstance.getPayload().startsWith(payload)) continue;
  ...
  public List<WorkflowInstance> getWorkflowInstances(
      Status wfStatus, DateTime startTimeLimit, DateTime endTimeLimit) throws ClientException {
   ...
   *
   * @param instanceURI the workflow instance uri
   * @param expectedStatus provide the status code that is expected, default is 200 OK.
  ...
  public List<HistoryItem> getWorkflowInstanceHistory(String instanceURI, int... expectedStatus)
    ...
    List<NameValuePair> params =
        Collections.<NameValuePair>singletonList(new BasicNameValuePair("workflow", instanceURI));
    SlingHttpResponse exec =
        doGet(
            "/libs/cq/workflow/content/console/history.json",
            params,
            HttpUtils.getExpectedStatus(SC_OK, expectedStatus));
   ...
   *
   * @param instanceURI path to the instance url
   * @param status status to be set, either {@link Status#ABORTED}, {@link Status#RUNNING} or {@link
   *     Status#SUSPENDED}
   * @param expectedStatus Expected HTTP status, otherwise 200 is assumed
   * @return the request executor if more info from response is required.
   * @throws ClientException if POST request has problems
   */
  public SlingHttpResponse setWorkflowInstanceStatus(
      String instanceURI, Status status, int... expectedStatus) throws ClientException {
    SlingHttpResponse exec =
        doPost(
            instanceURI,
