...
public class CQAssetsClient extends CQClient {
  ...
  /**
   * The default delay between polling for asset processing status, {@value #ASSET_PROCESSED_DELAY}
   * milliseconds.
   */
  ...
  /**
   * Uploads an <b>Asset</b> to the repository. Same as using {@code New File...} in the DAM admin
   * or when uploading a file below {@code Digital Assets} in the Site Admin.<br>
   * <br>
   ...
   * int...)}.<br>
   * <br>
   * This implementation will pick Direct Binary Access upload when available or fallback to the
   * original AEM implementation
   *
   ...
   * @throws ClientException if something fails during the request/response cycle
   */
  public SlingHttpResponse uploadAsset(
      String fileName,
      String resourcePath,
      String mimeType,
      String parentPath,
  ...
  /**
   * Wait for an asset to complete processing. The total timeout is {@value
   * #ASSET_PROCESSED_TIMEOUT} milliseconds. Polling the asset status occurs every {@value
   * #ASSET_PROCESSED_DELAY} milliseconds.
   *
   * @param assetPath Path to an asset
   * @return ProcessedAsset
   * @throws ClientException if something fails during the request/response cycle, or if the asset
   *     failed to process
  ...
  /**
   * Wait for an asset to complete processing. The asset state will be checked at least once. If the
   * timeout is 0 or less, then the asset state will be executed exactly once.
   *
   * @param assetPath Path to an asset
  ...
  /**
   * Retrieve any processing failures with an asset. Only able to find failures when Asset Compute
   * is enabled, will return an empty {@link List} otherwise.
   *
   * @param assetPath Asset path
  ...
  public List<String> getAssetsProcessedRenditions(String assetPath) throws ClientException {
    ...
    if (response.getCode() == HttpStatus.SC_OK) {
      ...
      while (fieldNames.hasNext()) {
        ...
        String primaryType =
            n.get("jcr:primaryType") != null ? n.get("jcr:primaryType").textValue() : null;
        if (primaryType != null) {
  ...
  private boolean checkDirectBinaryAccessSupport() throws ClientException {
    SlingHttpResponse response =
        doGet(ACP_LIST_CONTENT_DAM_FOLDER, HttpStatus.SC_OK, HttpStatus.SC_NOT_FOUND);
  ...
  private SlingHttpResponse uploadAssetDBA(
    ...
    if (partSize > uploadFile.getMaxPartSize()) {
      ...
      throw new ClientException(
          "InitiateUpload response requires a partSize that's larger than maxPartSize: "
              + r
              + ", fileSize: "
              + fileSize);
    }
   ...
   * @return initiate upload response with urls where to upload the asset, and a url to complete the
   *     asset
  ...
  private InitiateUploadResponse initiateUpload(String fileName, String parentPath, long fileSize)
    ...
    try {
      ...
      throw new ClientException(
          "Unable to parse JSON response for initiateUpload - "
              + " requestPath: "
              + requestPath
              + ", fileName: "
              + fileName
   ...
   */
  private void uploadAssetPart(
      String resourcePath, String mimeType, URI targetUri, long start, long size)
      throws ClientException {
    HttpPut request = new HttpPut(targetUri);
    request.setHeader(HttpHeaders.CONTENT_TYPE, mimeType);

    // we need to support retries which are more common with cloud blob storage (temporary 503s
    // etc.)
   ...
   */
  private SlingHttpResponse completeUpload(
      String completeUri,
      String fileName,
      String uploadToken,
      String mimeType,
   ...
   * @throws ClientException if something fails during the request/response cycle
   */
  private SlingHttpResponse uploadAssetViaServlet(
      String fileName,
      String resourcePath,
      String mimeType,
      String parentPath,
