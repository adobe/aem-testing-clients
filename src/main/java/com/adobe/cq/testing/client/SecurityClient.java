...
public class SecurityClient extends CQClient {
  ...
  /**
   * Get {@link AuthorizableManager} to get and create {@link Authorizable}s: User / Groups
   *
   ...
   */
  public void deleteAuthorizables(Authorizable[] authorizables, int... expectedStatus)
   ...
   */
  public User createUser(String userId, int... expectedStatus)
   ...
   * @throws InterruptedException to mark this method as "waiting"
   */
  public <T extends Group> User createUser(
      String userId, String password, T[] assignedGroups, int... expectedStatus)
      throws ClientException, InterruptedException {
    return createUser(userId, password, null, null, assignedGroups, expectedStatus);
  }

  /**
   * Creates a new user with a {@link com.adobe.cq.testing.client.security.Profile} and assigns him
   * to a list of given {@link Authorizable}s.
   *
   * @param userId the user ID for the new user.
   * @param password the password to be assigned.
   * @param intermediatePath the root path user will be created.
  ...
  public <T extends Group> User createUser(
      ...
      throws ClientException, InterruptedException {
    User user =
        User.createUser(this, userId, password, intermediatePath, profileMap, expectedStatus);
   ...
   * @param intermediatePath the root path user will be created.
   * @param profileMap profile properties to be set for the new user.
   * @param waitForIndexing whether or not to wait for indexing to be completed after creating the
   *     user
   * @param assignedGroups groups the user will be assigned to as {@link Authorizable}-Array.
   * @param expectedStatus list of allowed HTTP Status to be returned.
   * @param <T> group type
   ...
   * @throws ClientException If something fails during request/ response cycle
   * @throws InterruptedException to mark this method as "waiting"
   * @deprecated waitForIndexing is not used anymore and it's ignored. Use {@link
   *     #createGroup(String, String, String, String, Group[], int...)}.
   */
  @Deprecated
  public <T extends Group> User createUser(
      String userId,
      ...
      throws ClientException, InterruptedException {
    return createUser(
        userId, password, intermediatePath, profileMap, assignedGroups, expectedStatus);
  }
   ...
   * @throws InterruptedException to mark this method as "waiting"
   */
  public Group createGroup(String groupId, int... expectedStatus)
      throws ClientException, InterruptedException {
  ...
  public Group createGroup(
      ...
      throws ClientException, InterruptedException {
    Group group =
        Group.createGroup(this, groupId, intermediatePath, givenName, aboutMe, expectedStatus);
   ...
   *
   * @param expectedStatus list of allowed HTTP Status to be returned. If not set, http status 302
   *     (REDIRECT (to the requested resource)) is assumed.
