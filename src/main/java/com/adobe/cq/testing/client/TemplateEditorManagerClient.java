...
/**
 * Client to create and update the page template. To create a default template use {@link
 * #createDefaultTemplate(String configPath, String title, String Description)} To create a template
 * with give type use {@link #createTemplate(String configPath, String templateType, String title,
 * String Description)} To create Default Container on top level of a template use {@link
 * #createTopLevelDefaultContainer(String templatePath, String name)} To create a Default container
 * in a template use {@link #createDefaultContainer(String templatePath, String location, String
 * name)}
 */
public final class TemplateEditorManagerClient extends CQClient {

  private static final String TEMPLATE_CREATE_ENDPOINT =
      "/libs/wcm/core/content/sites/createtemplatewizard/_jcr_content";
  ...
  private static final String DEFAULT_CONTAINER_RESOURCE_TYPE =
  ...
  public String createDefaultTemplate(
      final String configPath, final String title, final String description)
  ...
  public String createTemplate(
    ...
    String content =
        doPost(TEMPLATE_CREATE_ENDPOINT, formEntry.build(), HttpStatus.SC_CREATED).getContent();
    return extractPath(content);
  }

  /**
   * Creates default layout container (responsivegrid) on the provided location of the provided
   * template
   *
   * @param templatePath: path of the template the layout container should be created, i.e.
   *     /conf/myConfig/settings/wcm/templates/myTemplate
   * @param location: the location the layout container should be created, i.e.
   ...
   * @throws ClientException if the request fails
   */
  public String createDefaultContainer(String templatePath, String location, String nameHint)
      throws ClientException {
    if (nameHint == null) nameHint = "responsivegrid";
    FormEntityBuilder formEntry =
        FormEntityBuilder.create()
            .addParameter(
                PARAM_COPY_FROM, "/libs/" + DEFAULT_CONTAINER_RESOURCE_TYPE + "/cq:template")
            .addParameter(PARAMETER_CHARSET, CHARSET_UTF8)
  ...
  /**
   * Creates a default layout container (responsivegrid) on structure top level of the provided
   * template
   *
   * @param templatePath: path of the template the layout container should be created, i.e.
   *     /conf/myConfig/settings/wcm/templates/myTemplate
   ...
   * @throws ClientException if the request fails
   */
  public String createTopLevelDefaultContainer(String templatePath, String nameHint)
      throws ClientException {
   ...
   * @throws ClientException if the request fails
   */
  public String importPolicy(final String configPath, final String jsonString)
      throws ClientException {
    return importJson(
   ...
   * @throws ClientException if the request fails
   */
  public void setStructureComponentLock(String componentPath, boolean isLocked)
      throws ClientException {
  ...
  /**
   * Set the policy mapping for a component. Note: The componentPath is the path to the component in
   * the template structure tree except for components in a unlocked layout container, here the
   * component path is the path to the layout container component in the template structure tree
   * plus the component resource type, i.e. for a foundation text component in unlocked main layout
   * container:
   ...
   * @param policyPath: the path of the policy to be set for the component, i.e.
   *     wcm/foundation/components/responsivegrid/default
  ...
  public String setComponentPolicy(String componentPath, String policyPath) throws ClientException {
    ...
    FormEntityBuilder form =
        FormEntityBuilder.create()
            .addParameter(
                PARAM_JCR_PRIMARY_TYPE, "nt:unstructured") // set primary type for folder node
    ...
    SlingHttpResponse exec =
        this.doPost(resourcePath, form.build(), HttpStatus.SC_OK, HttpStatus.SC_CREATED);
    return exec.getSlingPath();
  }

  /**
   * Get the resource type of the policy mapping node depending if the component is a page component
   * or not (note: page components use different resource type in policy mapping than all other
   * components)
   *
   * @param componentPath: the path of the component for which the policy should be set
  ...
  private String getPolicyMappingResourceType(String componentPath) {
    return componentPath.endsWith("/jcr:content")
        ? "wcm/core/components/policies/mappings"
