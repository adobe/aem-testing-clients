...
/**
 * The base client for all Multi Site Manager (MSM) related tests. It provides a core set of
 * commonly used MSM functions e.g. creating a blueprint / site / live copy. The MSM enables you to
 * create a site (called a live copy) based on another site (called a blueprint) and to actively
 * manage the relationships between the blueprint and the live copy. <br>
...
public class MSMClient extends CQClient {
  ...
  /**
   * Creates a blueprint. <br>
   * The blueprint makes it possible to define structure and content centrally. The structure and
   * content of the blueprint can then be used in the live copy
   *
  ...
  public SlingHttpResponse createBluePrint(
      String name, String title, String sitePath, int... expectedStatus) throws ClientException {
    // create blueprint
    String blueprintPath =
        createPage(name, title, BLUEPRINT_ROOT, "/libs/wcm/msm/templates/blueprint", expectedStatus)
            .getSlingPath();
    ...
    return doPost(
        postPath,
        FormEntityBuilder.create().addParameter("./sitePath", sitePath).build(),
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public SlingHttpResponse deleteBluePrint(String blueprintPath, int... expectedStatus)
      throws ClientException {
    return deletePage(new String[] {blueprintPath}, false, false, expectedStatus);
  }

  /**
   * Creates an new site. A site can be considered as 1:1 copy of an existing branch or page. Every
   * site can be defined as live copy which manage the relationships between the blueprint and the
   * live copy.
   *
  ...
  /**
   * Creates a live copy. A live copy can be created from an existing branch or page or a site that
   * is a live copy by using a blueprint.
   *
   * @param label the label/name of the new site
   * @param title the title of the new site
   * @param destPath the location where the new site gets created
   ...
   * @param rolloutConfigs the rollout configuration for the live copy
   * @param missingPages pages which are not rolled out yet (missing) but should be part of the
   *     rollout
   * @param excludeSubPages true if subpages of missing pages are excluded (subpages of missing
   *     pages are not par of rollout)
   * @param expectedStatus list of allowed HTTP Status to be returned. If not set, http status 200
   *     (OK) is assumed.
   * @return Sling response
   * @throws ClientException If something fails during request/response cycle
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public SlingHttpResponse editRelationship(
      String target,
      String sourcePath,
      boolean deep,
      String[] rolloutConfigs,
    ...
    return doPost(
        postPath, entityBuilder.build(), HttpUtils.getExpectedStatus(SC_OK, expectedStatus));
  }
   ...
   * @param liveActions names of the LiveActions to be contained in the RolloutConfig @return the
   *     Path to the newly created RolloutConfig
   * @return the path to the rollout config
   * @throws ClientException in case of Errors during RolloutConfiguration creation
   */
  public String createRolloutConfig(
      String name, String above, String trigger, String... liveActions) throws ClientException {
    ...
    if (above != null) {
      UrlEncodedFormEntity entity =
          FormEntityBuilder.create().addParameter(":order", "before " + above).build();
