...
public class LogTracerRule extends ExternalResource {
  ...
  private List<Header> tracerHeaders =
      Arrays.<Header>asList(
          ...
          new BasicHeader(
              "Sling-Tracer-Config",
              "com.day;level=error,com.adobe;level=error,org.apache;level=error"));
  ...
  public LogTracerRule(
      Instance quickstartRule, int recordingCacheSizeInMB, int recordingCacheDurationInSecs) {
    super();
    this.quickstartRule = quickstartRule;
  ...
  /**
   * Parse the tracer request id from the given response object and return the corresponding tracer
   * report from the tracer servlet as JSON object.
   *
   ...
   */
  public JsonNode getTracerReport(SlingHttpResponse response)
  ...
  /**
   * Parse the tracer request id from the given response object and return the corresponding logs
   * from the tracer servlet as JSON array.
   *
   * @param response SlingHttpResponse object
  ...
  /**
   * Parse the tracer request id from the given response object and check if there are any entries
   * in the corresponding log. Marks the test case from which this method was called as failed if
   * there are any log entries.
   *
   * @param response SlingHttpResponse object
   * @throws ClientException if the request fails
   * @throws InterruptedException to mark this method as waiting
  ...
  private class LogReportPoller extends Polling {
    ...
    public Boolean call() throws ClientException {
      SlingHttpResponse logResponse =
          this.client.doGet("/system/console/tracer/" + requestId + ".json", SC_OK);
