...
public class Start extends AbstractComponent {
  ...
  /**
   * We override the default create call, so we can capture the reference to the end component. The
   * end component is created right after the start component.
   *
  ...
  /**
   * Defines what type of action should be executed upon form submission. actions are implemented as
   * components so this value must be a handle to a node in the repository that has
   * sling:resourceType set to foundation/components/form/action.
   *
  ...
  /**
   * Defines a form validation resource type if you want to validate the entire form (instead of
   * individual fields).
   *
  ...
  /**
   * Get the action property value (which is the action store content path) from this start
   * component.
   *
  ...
  /**
   * this method will 'emulate' the submit of this form by rebuilding the multipart/form-data
   * request, collecting all the values from the form fields found between the start and end
   * component
   *
   * @throws ClientException if the request fails
   * @throws InterruptedException to mark this method as waiting
  ...
  /**
   * Create a MultiPartEntity out of the all the values of the Form components which are between
   * this Start component and its End component in the page paragraph list. This MultiPartEntity is
   * used to submit the form data to the server. Accepted as Form values are: - String for single
   * value parameter if the parameter name is the same as the component name - String Array for
   * multi value parameters if the parameter name is the same as the component name - File for File
   * Uploads - Map of Strings for Form components which are sending several parameters (e.g Captcha)
   *
  ...
  /**
   * The value we want to submit for a form field. The value is only submitted if the corresponding
   * form field component still exists at time when {@link #submit()} gets called and is located
   * between the form start and end component.
   *
   * <p>If the the value is not set or set to null at the time of submitting then the form field is
   * not added at all.
   *
   * <p>If set to an empty String, it will submitted as such.
   *
