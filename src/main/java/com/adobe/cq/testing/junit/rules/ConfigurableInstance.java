...
public class ConfigurableInstance extends ExistingInstance {
  ...
  public ConfigurableInstance(boolean forceBasicAuth, boolean forceIndexLaneDetection) {
    ...
    LOG.info(
        "Using instances with config: [authentication method: {}, indexing lanes detection: {}]",
        (useLoginToken ? "LoginToken" : "Basic"),
        detectIndexLanes);
  }

  @Override
  public <T extends SlingClient> T newClient(
      Class<T> clientClass, String user, String pass, BuilderCustomizer... customizers) {
  ...
  public <T extends SlingClient.InternalBuilder> T customize(T builder) {
    if (useLoginToken) {
      ...
      builder
          ...
          .addInterceptorFirst(new FormBasedAuthInterceptor("login-token"));
      return builder;
    } else {
