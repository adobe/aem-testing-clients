...
public abstract class AbstractAuthorizable implements Authorizable {
  ...
  /**
   * Constructor for an {@link Authorizable}, default path to authorizable is expected
   *
  ...
  public String getJsonAsString(String propsFilter, int... expectedStatus) throws ClientException {
    List<NameValuePair> params =
        Collections.<NameValuePair>singletonList(
            new BasicNameValuePair(
                "props", (propsFilter != null) ? propsFilter : Authorizable.WILDCARD));
  ...
  public String getUserPropertiesJsonAsString(int... expectedStatus) throws ClientException {
    return client
        .doGet(getHomePath() + SELECTOR_USERPROPERTIES + ".json", expectedStatus)
        .getContent();
  }

  public static boolean exists(SecurityClient client, String authorizableId)
  ...
  public <T extends SecurityClient> Authorizable create(
      T client,
    ...
    return client
        .getManager()
        .create(client, authorizableClass, authorizableId, formParameters, expectedStatus);
  }

  public Map<String, Authorizable> getMemberOf() throws ClientException, InterruptedException {
    JsonNode authorizableNode =
        JsonUtils.getJsonNodeFromString(getJsonAsString(Authorizable.MEMBER_OF, SC_OK));
  ...
  public Map<String, Authorizable> getMembers() throws ClientException, InterruptedException {
    JsonNode authorizableNode =
        JsonUtils.getJsonNodeFromString(getJsonAsString(Authorizable.MEMBERS, SC_OK));
    JsonNode propsNode = null;
    if (authorizableNode != null) {
  ...
  public Map<String, Authorizable> getImpersonators() throws ClientException, InterruptedException {
    JsonNode authorizableNode =
        JsonUtils.getJsonNodeFromString(getJsonAsString(Authorizable.IMPERSONATORS, SC_OK));
    JsonNode propsNode = null;
    if (authorizableNode != null) {
   ...
   */
  public SlingHttpResponse doPost(FormEntityBuilder formParameters, int... expectedStatus)
      throws ClientException {
   ...
   * @throws ClientException if the request failed
   */
  public SlingHttpResponse doGet(List<NameValuePair> parameters, int... expectedStatus)
      throws ClientException {
   ...
   */
  public static Class<? extends AbstractAuthorizable> getAuthorizableClass(
   ...
   * @throws ClientException if the request failed
   */
  private String getAuthorizablePath(String authorizableId)
      throws ClientException, InterruptedException {
   ...
   */
  private String getAuthorizableType(String authorizableId)
      throws ClientException, InterruptedException {
    return getAuthorizableNodeWithRetry(authorizableId).path(Authorizable.TYPE).textValue();
  }

  private JsonNode getAutorizablesWithRetry(final String query)
      throws ClientException, InterruptedException {
  ...
  private static JsonNode getAuthorizables(SecurityClient client, final String query)
      throws ClientException {
   ...
   * @throws ClientException if the request failed
   */
  private JsonNode getAuthorizableNodeWithRetry(String authorizableId)
      throws ClientException, InterruptedException {
  ...
  /**
   * Get home path of the authorizable (user/group) in repository. Either user is created in a user
   * defined structure (intermediatePath is set while creation) or within the default structure
   *
   * @param authorizableId the ID of the authorizable
   ...
   * @throws ClientException if the request failed
   */
  public static String buildAuthorizablePath(
      String authorizableId, String rootPath, String intermediatePath) throws ClientException {
  ...
  /**
   * Get url of the authorizable (user/group) in repository. Either user is created in a user
   * defined structure (intermediatePath is set while creation) or within the default structure
   *
   * <p>JCR escaping works similar to URL encoding, meaning that special chars doGet %-encoded. so
   * to avoid % being interpreted by the URL as an URL Encoding we must URL encode the % to doGet a
   * valid URL
   *
  ...
  /**
   * Build list of {@link Authorizable}s as Map: key = id of authorizable.
   *
   ...
   * @throws InterruptedException to mark this method as "waiting"
   */
  private Authorizable buildAuthorizable(JsonNode authorizableNode)
      throws ClientException, InterruptedException {
    // assert valid json
