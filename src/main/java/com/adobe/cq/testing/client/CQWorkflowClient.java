...
public class CQWorkflowClient extends WorkflowClient {
  ...
  public CQWorkflowClient(CloseableHttpClient http, SlingClientConfig config)
   ...
   */
  public SlingHttpResponse createNewModelPage(
      String modelName, String modelTitle, int... expectedStatusRange) throws ClientException {
    // create the model page
    return cqClient.createPage(
        modelName, modelTitle, ROOT_PATH_MODEL, CQ_MODEL_PAGE_TEMPLATE, expectedStatusRange);
  }
   ...
   * @throws ClientException if the request fails
   */
  public SlingHttpResponse saveModel(String modelPath, int... expectedStatus)
      throws ClientException {
   ...
   */
  public void createWorkflowLauncher(
      String workflowId, String path, String nodeType, EventType eventType) throws ClientException {
    HttpEntity form =
  ...
  /**
   * Edit one property of an existing workflow launcher identified by the launcher configuration
   * name
   *
   * <p>The rest of the properties on the workflow launcher stay the same
   *
  ...
  /**
   * Edit an existing workflow launcher identified by the launcher configuration name with a set of
  ...
  public void editWorkflowLauncher(String launcherId, Map<String, String> propMap)
    ...
    if (!propMap.keySet().contains("excludeList")) form.addParameter("excludeList", "");
  ...
  public Map<String, String> getWorkflowLauncher(String launcherId) throws ClientException {
    ...
    if (null == launcherNode) return null;
   ...
   * @throws ClientException if the request fails
   */
  public void enableDisableWorkflowLauncher(String launcherId, boolean enabled)
      throws ClientException {
   ...
   *
   * @param workflowModel the path of the workflow model. These are stored at <code>
   *     /etc/workflow/models/</code>. The model path to be provided can be relative (or absolute)
   *     to this path
   * @return <code>True</code> if the workflow is transient in nature, <code>False</code> otherwise.
   * @throws ClientException if it fails to find the specified workflow model.
   */
  public boolean isWorkflowTransient(String workflowModel) throws ClientException {
    workflowModel =
        workflowModel.startsWith("/etc/workflow/models/")
            ? workflowModel
            : "/etc/workflow/models/" + workflowModel;
    workflowModel =
        workflowModel.endsWith("/jcr:content") ? workflowModel : workflowModel + "/jcr:content";
    JsonNode json = this.adaptTo(JsonClient.class).doGetJson(workflowModel, 0);
    return (!json.path("transient").isMissingNode())
        && (json.get("transient").asText().equalsIgnoreCase("true"));
  }
}
