...
public abstract class AbstractComponent {
   ...
   *
   * @deprecated - use the component's {@link #getLocation()} method as this value is not applicable
   *     to all websites
   */
  @Deprecated public static final String DEFAULT_LOCATION = "/jcr:content/par/";
  ...
  /**
   * Reference to the {@link com.adobe.cq.testing.client.FoundationClient } that created this
   * instance
   */
   ...
   */
  public SlingHttpResponse create(String order, int... expectedStatus)
    ...
    HttpUtils.verifyHttpStatus(
        exec, errorMessage, HttpUtils.getExpectedStatus(SC_CREATED, expectedStatus));
  ...
  /**
   * Returns the FormEntityBuilder used for the create request in {@link #create(String,int...)}.
   * This method can be overridden in a subclass to extend the parameters that need to sent with the
   * creation request.
   *
   * @return An FormEntityBuilder with {@code _charset_},{@code ./sling:resourceType} and {@code
   *     :nameHint} set.
   * @param order Defines where the component should be added in relation to its siblings. Possible
   *     values are {@code first}, {@code last}, {@code before [nodeName]}, {@code after
   *     [nodeName]}.
  ...
  /**
   * Submits all editable values to the server. Same as pressing the {@code OK} button on the edit
   * dialog of a component.
   *
   * @param expectedStatus list of allowed HTTP Status to be returned. if not set, status 200 is
   *     assumed
   * @return Sling response
   * @throws ClientException If something fails during request/response cycle
   * @throws InterruptedException to mark this method as waiting
   */
  public SlingHttpResponse save(int... expectedStatus)
      throws ClientException, InterruptedException {
    // Start the form
    ...
    SlingHttpResponse exec =
        client.doPost(
            componentPath, form.build(), HttpUtils.getExpectedStatus(SC_OK, expectedStatus));
   ...
   * @throws InterruptedException to mark this method as waiting
   */
  public SlingHttpResponse reorder(String order, int... expectedStatus)
      throws ClientException, InterruptedException {
    // Start the form
  ...
  /**
   * Returns the name of the component node. Gets preset with the nameHint passed in the constructor
   * and updated if required after the create() request.
   *
  ...
  /**
   * Returns the component wrapper instance for the following sibling or null if this component is
   * already the last one. It looks for the sling:resourceType property an tries to find a matching
  ...
  /**
   * Returns the component path for the following sibling or null if this component is already the
   * last one.
   *
   ...
   * @return an Component wrapper or null if it has no child components or the component type is
   *     unknown
  ...
  /**
   * Returns the component wrapper instance for the previous sibling or null if this component is
   * already the first one. It looks for the sling:resourceType property an tries to find a matching
   * component wrapper that has been registered with the ComponentClient (see {@link
   * com.adobe.cq.testing.client.ComponentClient#registerComponent(String, Class)})
   *
   * @param <T> component type
  ...
  /**
   * Returns the component path for the previous sibling or null if this component is already the
   * last one.
   *
   * @return The component path to the following sibling.
   * @throws ClientException if something goes wrong during request/response of json.
