...
/**
 * The base client for all notifications related tests. It provides a core set of commonly used
 * notification functions e.g. list / approve / delete <br>
...
public class NotificationClient extends CQClient {
  ...
  public static final String NOTIFICATIONS_SUBSCRIPTIONS_PATH =
      "/home/users/geometrixx/author/wcm/notification/config/subscriptions";
  ...
  public NotificationClient(CloseableHttpClient http, SlingClientConfig config)
  ...
  /**
   * Get the list of available action to be notified for. For now this list is hardcoded in a
   * javascript library file which is loaded by the UI
   *
  ...
  public ArrayList<String> getAvailableNotificationAction() throws ClientException {
    ...
    String notifPart =
        widgetJs.substring(
            widgetJs.indexOf("CQ.wcm.NotificationInbox"),
            widgetJs.indexOf("CQ.Ext.reg(\"notificationinbox\", CQ.wcm.NotificationInbox);"));

    Pattern p =
        Pattern.compile(
            "CQ\\.wcm\\.NotificationInbox = .+var options = (\\[.+\\]);.+", Pattern.DOTALL);
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public ArrayList<Notification> getNotificationMessages(int start, int limit)
      throws ClientException {
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public SlingHttpResponse configNotifications(
      String channel, String[] actions, String packages, int... expectedStatus)
      throws ClientException {
    // prepare the form
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public SlingHttpResponse performAction(
      Notification notification, String action, int... expectedStatus) throws ClientException {
    FormEntityBuilder formEntry = FormEntityBuilder.create();
