...
public class OffloadingBrowserClient extends CQClient {
  ...
  public static final String OFFLOADING_BROWSER_JSON_LIST_URL =
      "/libs/granite/offloading/content/view/views/wrapper/items/servletComponentPlaceholder.list.json";
  public static final String OFFLOADING_CONFIG_URL =
      "/libs/granite/offloading/content/view/views/wrapper/items/servletComponentPlaceholder.config.html";
  ...
  public OffloadingBrowserClient(CloseableHttpClient http, SlingClientConfig config)
      throws ClientException {
    super(http, config);
  }

  public OffloadingBrowserClient(URI serverUrl, String user, String password)
      throws ClientException {
  ...
  public Set<OffloadingInstanceConfiguration> getAllInstances() throws ClientException {
    ...
    for (JsonNode topicNode : topics) {
      ...
      Set<OffloadingInstanceConfiguration> topicInstances =
          instancesFromJSONArray(instancesNode, topicNode.get("topic").textValue().trim());
  ...
  /**
   * Enables/ disables a topic for the given slingId. This method is asynchronous, it does not wait
   * for the configuration change to be applied.
   *
   * @param slingId id of the instance
   ...
   */
  public void enableDisableTopic(String slingId, String topic, boolean enable)
      throws ClientException {
    ...
    LOG.debug(
        "URL:{}, slingId:{}, topic:{}, enable: {}", OFFLOADING_CONFIG_URL, slingId, topic, enable);
    this.doPost(OFFLOADING_CONFIG_URL, form.build(), HttpStatus.SC_OK);
  }

  /**
   * Enables/ disables a topic for the given slingId. This method checks periodically until the
   * topic was enabled/disabled in the OffloadingBrowser with timeout. If the change was not visible
   * after timeout, throws ClientException
   *
   * @param slingId id of the instance
   * @param topic topic to enable/disable
   * @param enable enable or disable
   * @param timeout number of milliseconds to wait for the topic configuration to be updated
   * @throws ClientException if the action did not register with the OffloadingBrowser before
   *     timeout
   * @throws InterruptedException if the method was interrupted
   */
  public void enableDisableTopicWithWait(
      final String slingId, final String topic, final boolean enable, long timeout)
      throws ClientException, InterruptedException {

    enableDisableTopic(slingId, topic, enable);

    // wait for the action to be registered
    Polling topicPolling =
        new Polling() {
          @Override
          public Boolean call() throws Exception {
            OffloadingInstanceConfiguration instance = getInstance(slingId);
            return (enable && instance.topics.contains(topic))
                || (!enable && !instance.topics.contains(topic));
          }
        };
    try {
      ...
      throw new ClientException(
          enable
              ? "Enabling"
              : "Disabling" + " the topic did not register in the Offloading Browser.");
    }
  }

  private Set<OffloadingInstanceConfiguration> instancesFromJSONArray(
      JsonNode instancesNode, String topicName) {
    Set<OffloadingInstanceConfiguration> instances = new HashSet<>();
    for (JsonNode instanceNode : instancesNode) {
      ...
      if (instanceNode.get("topicEnabled") != null
          && instanceNode.get("topicEnabled").booleanValue()) {
