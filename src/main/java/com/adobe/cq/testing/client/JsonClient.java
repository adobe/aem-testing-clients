...
public class JsonClient extends CQClient {
  ...
  public JsonNode getPages(String parentPath) throws ClientException {
    URLParameterBuilder params =
        URLParameterBuilder.create().add("tidy", "true").add("predicate", "page");
  ...
  /**
   * Get all references used and its status in a given page <br>
  ...
  /**
   * Queries the list of page references for a given page, as it is used to display in the move
   * dialog.
   *
  ...
  public JsonNode getMoveReferences(String pagePath, int... expectedStatus) throws ClientException {
    ...
    SlingHttpResponse exec =
        doGet(
            postPath,
            URLParameterBuilder.create().add("path", pagePath).getList(),
  ...
  /**
   * Get list of available languages for a page. <br>
  ...
  public JsonNode getLanguages(String sitePath, boolean deep) throws ClientException {
    ...
    SlingHttpResponse exec =
        doGet(
            getPath,
            URLParameterBuilder.create().add("deep", Boolean.toString(deep)).getList(),
            SC_OK);
    return JsonUtils.getJsonNodeFromString(exec.getContent());
  }
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public JsonNode getVersions(String pagePath, boolean showChildren, int... expectedStatus)
    ...
    SlingHttpResponse exec =
        doGet(
            ...
            URLParameterBuilder.create()
                ...
                .add(
                    "date",
                    date == null ? null : TestUtil.ISO_DATETIME_TIME_ZONE_FORMAT.format(date))
                .getList(),
            HttpUtils.getExpectedStatus(SC_OK, expectedStatus));
  ...
  /**
   * Get user generated pages in the {@code user generated content} (ucg) space, currently under
   * {@code /content/usergenerated}. <br>
   * In case path is not valid, an empty pages array will be returned
   *
   ...
   *     (nt:hierarchy- node, not hidden, no file
   ...
   * @throws ClientException If something fails during request/response cycle
   */
  public JsonNode getUserGeneratedPages(
